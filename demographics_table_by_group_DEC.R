...
NEW %>%
  tab_cells(NEW$gender_male,
            ...
            NEW$ADHD_d) %>%
  tab_cols() %>%
  tab_significance_options(digits = 4) %>%
  tab_stat_mean_sd_n() %>%
...
sink('Site_differences.txt')
kruskal.test(prenat_mateduc_college, data=NEW)
kruskal.test(prenat_mateduc_uni ~ Hamilton, data=NEW)
...
kruskal.test(Smoking_During_Pregnancy.1 ~ Hamilton, data=NEW)
kruskal.test(Alcohol_During_Pregnancy ~ Hamilton, data=NEW)
kruskal.test(Alcohol_During_Pregnancy.1 ~ Hamilton, data=NEW)
kruskal.test(Size_of_family_in_household_inc_index_child ~ Hamilton, data=NEW)	
kruskal.test(Size_of_family_requires_sibs_to_live_in_house ~ Hamilton, data=NEW)
kruskal.test(birth_size_percent2_x ~ Hamilton, data=NEW) 
kruskal.test(gestation_age_wks~ Hamilton, data=NEW)	
kruskal.test(gestation_age_days ~ Hamilton, data=NEW)
sink()

sink('table_by_site.txt')

NEW %>%
  tab_cells(NEW$prenat_mateduc_college,
            NEW$prenat_mateduc_uni,
            NEW$QuintMat_w,	
            NEW$QuintSoc_w,
            NEW$Smoking_During_Pregnancy,
            NEW$Smoking_During_Pregnancy.1,
            NEW$Alcohol_During_Pregnancy,
            NEW$Alcohol_During_Pregnancy.1,
            NEW$Size_of_family_in_household_inc_index_child,
            NEW$Size_of_family_requires_sibs_to_live_in_house,
            NEW$birth_size_percent2_x,
            NEW$gestation_age_wks,
            NEW$gestation_age_days) %>%
  tab_cols(total(), NEW$Hamilton
  ) %>% 
  tab_significance_options(digits = 4) %>%
  tab_stat_mean_sd_n() %>%
...
A	LIFE	CONTEXT	PARENTAL	INTERPERSONAL	GEN	SOM	DEP	ANHED	ANX	
Look_at_baby_frequency	Look_away_from_baby_frequency	Look_overf	Joint_focus_frequency	nursef	rockf	cryf	infant_Smiled	Reachf	
Toy_playf	gawayf	humf	sgamesf	swtoyf	kissf	phywotyf	matvocalf	obscuredf	pokef	mouthf	groomf	strokef	laughf	vocalizef	phytoyf	imitatef	
MBQSR_6M	Ainsworth_Sensitivity_6M.y	Ainsworth_Cooperation_6M	Ainsworth_Availability_6M	Ainsworth_Acceptance_6M	MBQSR_18M	Ainsworth_Sensitivity_18M.y	
Ainsworth_Cooperation_18M	Ainsworth_Availability_18M	Ainsworth_Acceptance_18M	rockp	sgamesp	swtoyp	phywotyp	phytoyp	looatp	awayp.y	overp	
jointp	cryp	insmilep	reachp	pltoyp	
gawayp	vocalizep	kissp	pokep	mouthp	strokep	hump	matvocalp	laughp	imitatep	awayf	awayd	awayp.x	Ainsworth_Sensitivity_6M.x	Ainsworth_Sensitivity_18M.x
