...
get_coef_mean = function(boot_list){
  ...
  for (i in 1:length(boot_list)){
    ...
    coef_mean[j] =+ boot_list[[i]]$coef[[j]]
    }
  }
  coef_mean/length(boot_list)
  return(coef_mean)
}
...
####CI by hand with mean estimate plus minus 1.96x(standard deviation/ squareroot of n (sample size))
